    ...
    module.exports = function (apiKey, outputFormat) {
        return function (parameters, callback) {
            ...
            if (!parameters.duration) 
            	{ callback(new Error("Event duration is required"));
            	  }
            if (parseInt(parameters.duration) !== parameters.duration) 
            	{ callback(new Error("Event duration must be an integer"));
                  	}
            if (!parameters.summary) 
            	{callback(new Error("Event summary is required"));
                   	}
            if (!parameters.reference)
            	{callback(new Error("Place reference is required"));
            		}
            var options = {
                method: "POST",
